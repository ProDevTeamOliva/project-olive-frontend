...
const Post = ({ post }) => {
  const property = {
    ...
    postType: "Public",
    content:
      "Cześć, właśnie robię stronę w REACT-cie 🤪 bsugbrdugbdugdrhjbrkuhkgrbudgveurygvrubg ubudrb ugb dubg udrgb dubr gubrj db g jdbryg dybrgjy dbrjg bvudrbgudbrgudrbgudbrygjdgousergyfuosguydgdgrdgdrgdrgdgdrgdfgbdfjhgbdfukgbdfgbdubgriygbdigybrdigbdigbdrigbdigrbdrigbdigrbidbgidbgid",
    likes: 12,
  };
    ...
    >
      <Box p="6">
        ...
        >
          ...
          <Box tex="right" color="gray.500">
            {getPostTypeIcon(property.postType) ? (
              <Box display="flex">
                <Image
                  marginRight="10px"
                  boxSize="25px"
                  src={getPostTypeIcon(property.postType)}
                  borderRadius="full"
                ></Image>{" "}
                {property.postType}
              </Box>
