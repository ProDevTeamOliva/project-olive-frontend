...
import { InputGroup, Box, Center, List, ListItem, Spinner, Icon } from '@chakra-ui/react';
...
const SearchBar = () => {
	...
	const getResultsToRender = () => {
		if(inputValue.length > 2 && !loading && suggestions.length > 0){
			return(
				suggestions.map(({ id, nameFirst, nameLast, avatar }) => {
                  return (
                    <LinkSearch
                      key={id}
                      id={id}
		...
		if(loading){
			return(
				<ListItem p={3} borderBottom={'2px solid light-white'} fontFamily='Ubuntu' color={'black'}>
					<Spinner/>&nbsp;&nbsp;
					<span>Ładowanie ...</span>
				</ListItem>
			);
